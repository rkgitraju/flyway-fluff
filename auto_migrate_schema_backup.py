...
if __name__ == "__main__":
    ...
    if generated_file is not None :
        user_input = input("Do you want to execute this migration on the target database now? (yes/no): ").lower().strip()
        
        if user_input == 'yes':
            success = run_flyway_migration(generated_file)
            if not success:
                print(f"🛑 Migration failed. Check console output. Generated file was automatically cleaned up.")
        else:
            delete_generated_file(generated_file)
            print(f"❌ Migration cancelled by user. The file {os.path.basename(generated_file)} remains for manual review.")