...
# --- New Function ---
def wait_for_db(db_url, timeout=30, attempts=5):
    """Waits for the database specified by the URL to accept connections."""
    print(f"⌛ Waiting for database at {db_url.split('@')[-1]}...")
    
    # Simple URL parsing for psycopg2 connection
    match = re.search(r'postgresql://(?P<user>[^:]+):(?P<password>[^@]+)@(?P<host>[^:]+):(?P<port>\d+)/(?P<dbname>.+)', db_url)
    if not match:
    ...
    for attempt in range(attempts):
        try:
            conn = psycopg2.connect(
                ...
                connect_timeout=timeout // attempts 
            )
            conn.close()
            ...
            if attempt < attempts - 1:
                print(f"⚠️ Connection failed on attempt {attempt + 1}. Retrying in 5 seconds...")
                time.sleep(5)
            else:
                print(f"❌ Database connection failed after {attempts} attempts. Error: {e}")
                sys.exit(1)
    return False
...
def run_migra_and_generate_script():
    ...
    wait_for_db(SOURCE_DB_URL)
    wait_for_db(TARGET_DB_URL)
